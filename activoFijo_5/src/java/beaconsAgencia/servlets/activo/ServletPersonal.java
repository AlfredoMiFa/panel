 ...
 */
@WebServlet(name = "ServletPersonal", urlPatterns = {"/beaconsAgencias/Personal.do"})
        ...
        if(session.getAttribute(getServletContext().getInitParameter("vsi")+"SESION")==null){
            ...
            try {
                if(request.getParameter("modo")==null) {
                    ...
                    String order="desc"; 
                    String filter="where ";
                    String ids = PersonalEJBLocal.personal(usuario.getIdOficina().getIdOficina().toString());
                    String idsOficinas = OficinasEJB.oficinas(usuario.getIdEmpresa().getIdEmpresa().toString());
                    if(usuario.getGrupo().equals("ADMINISTRADOR")) {
                        for(int i=0;i<1;i++) {
                            filter += " ID_OFICINA IN("+idsOficinas+")";
                            i++;
                        }
                    }
                    if(usuario.getGrupo().equals("SUPERVISOR")) {
                        for(int i=0;i<1;i++) {
                            filter += " ID_PERSONAL IN("+ids+")";
                            i++;
                        }
                    }
                    ...
                    if(request.getParameter("filter[logic]")!=null) {
                        ...
                        {                            
                            if(mFiltroDoble[i]) {                             
                                ...
                                if(field.equals("ID_ESTADO") || field.equals("ID_CIUDAD") || field.equals("nombre") || field.equals("APELLIDO_PATERNO") ||
                                        field.equals("APELLIDO_MATERNO") || field.equals("EMAIL") || field.equals("calle") || field.equals("colonia")
                                        || field.equals("cpp") || field.equals("NUMERO_TELEFONO"))
                    ...
                    if(modo.equals("eliminar")) {
                        ...
                        response.getWriter().print(PersonalEJBLocal.generaCombo(request.getParameter("idDepartamento"),""));
