...
import beaconsAgencia.ejb.ImagenesFacadeLocal;
 ...
 */
@WebServlet(name = "ServletImagen", urlPatterns = {"/beaconsAgencias/Imagen.do"})
public class ServletImagen extends HttpServlet {

    @EJB
    ImagenesFacadeLocal ImagenesEJBLocal;
    @EJB
    ActivosFacadeLocal ActivoEJBLocal;
    /**
            ...
            try {
                ...
                if ( !isMultipart )
                {
                    ...
                    if(request.getParameter("modo")==null) {
                        ...
                        response.getWriter().print(ImagenesEJBLocal.select(rows,page,sort,order,filter));
                    }else {
                        String modo=request.getParameter("modo");
                        String id=request.getParameter("idImagenes");
                        Imagenes imagen = new Imagenes();
                        if(modo.equals("eliminar")) {
                            ImagenesEJBLocal.remove(ImagenesEJBLocal.find(Integer.parseInt(id)));
                            response.getWriter().print("{\"success\":true,\"title\":\"El registro se ha eliminado con exito.\",\"msg\":\"\"}");
                        }else if(modo.equals("nuevo") || modo.equals("editar")) {
                            if(!id.equals("") && id!=null)
                                imagen.setIdImagenes(Integer.parseInt(id));
                            imagen.setIdActivo(ActivoEJBLocal.find(Integer.parseInt(request.getParameter("idActivo"))));
                            imagen.setDescripcion(request.getParameter("descripcion"));
                            response.getWriter().print(ImagenesEJBLocal.persistir(imagen,modo));
                            }else if(modo.equals("getImage")) {
                            Imagenes img = ImagenesEJBLocal.find(Integer.parseInt(request.getParameter("id")));                        
                            response.setContentType("image/png");
                            if(img.getImagen()!=null)
                            {
                                response.getOutputStream().write(img.getImagen());
                            }else{
                                response.setContentType("text/html;charset=UTF-8");
                                response.getWriter().print("El registro no cuenta con una foto.");
                            }
                        }
                    } 
                }else {
                    ...
                    try
                    {
                        ...
                        while ( iter.hasNext() )
                        {
                            ...
                            }else if(fieldName.equals( "idImagenes" ))
                                idImagen=Integer.parseInt(Streams.asString(item.openStream()));
                        }
                        if(idImagen!=null) {
                            response.getWriter().print(ImagenesEJBLocal.subirImagen(bytes, idImagen));
                        }else
                            response.getWriter().print("{\"error\":\"Error al realizar la operacion.\"}");
                    }
