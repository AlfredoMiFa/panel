 ...
 * @author jgonzalezc
 */
@WebServlet(name = "ReglaSeguridadDetalleServlet", urlPatterns = {"/catalogos/generals/reglaSeguridadDetalle.do"})
public class ReglaSeguridadDetalleServlet extends HttpServlet {
        ...
        if(session.getAttribute(getServletContext().getInitParameter("vsi")+"SESION")==null)
        {
            ...
            try {
                ...
                if(request.getParameter("modo")==null) {
                    int rows=10;
                    int page=1;   
                    String sort="CORE_REGLA_SEGURIDAD_DETALLE_ID"; 
                    String order="desc"; 
                    String filter="";
                    ...
                    response.getWriter().print(reglaSeguridadDetalleEJBLocal.select(rows,page,sort,order,filter));                        
                }else {
                    String modo=request.getParameter("modo");
                    String id=request.getParameter("coreReglaSeguridadDetalleId");
                    CoreReglaSeguridadDetalle reglaSeguridadDetalle=new CoreReglaSeguridadDetalle();
                    if(modo.equals("eliminar")) {
                        reglaSeguridadDetalleEJBLocal.remove(reglaSeguridadDetalleEJBLocal.find(Integer.parseInt(id)));
                        ...
                        if(id!=null && !id.equals(""))
                            reglaSeguridadDetalle=reglaSeguridadDetalleEJBLocal.find(Integer.parseInt(id));
                        else{
                            reglaSeguridadDetalle.setCoreReglaSeguridadDetalleId(null);
                            reglaSeguridadDetalle.setCoreReglaSeguridadId(reglaSeguridadEJBLocal.find(Integer.parseInt(request.getParameter("coreReglaSeguridadId"))));
                        }
                        reglaSeguridadDetalle.setNombre(request.getParameter("nombre").toUpperCase());
                        reglaSeguridadDetalle.setCatalogo(request.getParameter("catalogo").replaceAll("\"","'"));
                        reglaSeguridadDetalle.setTipo(request.getParameter("tipo"));
                        reglaSeguridadDetalle.setValorIni(request.getParameter("valorIni"));
                        reglaSeguridadDetalle.setValorFin(request.getParameter("valorFin"));
                        response.getWriter().print(reglaSeguridadDetalleEJBLocal.persistir(reglaSeguridadDetalle,modo));                          
                    }
                }   
    ...
    private String fieldParse(String campo){
            switch (campo) {
                case "coreReglaSeguridadDetalleId":
                    return "CORE_REGLA_SEGURIDAD_DETALLE_ID";
                case "coreReglaSeguridadId":
