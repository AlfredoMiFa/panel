 ...
 * @author jgonzalezc
 */
@WebServlet(name = "DerechoSistemaServlet", urlPatterns = {"/catalogos/generals/derechoSistema.do"})
public class DerechoSistemaServlet extends HttpServlet {
    @EJB
    CoreDerechoSistemaFacadeLocal coreDerechoQvEJBLocal;
        ...
        if(session.getAttribute(getServletContext().getInitParameter("vsi")+"SESION")==null)
        {
            ...
            try {
                if(request.getParameter("modo")==null) {
                    int rows=10;
                    int page=1;   
                    String sort="CORE_DERECHO_SISTEMA_ID"; 
                    String order="desc"; 
                    String filter="";
                    ...
                    response.getWriter().print(coreDerechoQvEJBLocal.select(rows,page,sort,order,filter));
                }else {
                    String modo=request.getParameter("modo");
                    String id=request.getParameter("coreDerechoSistemaId");
                    CoreDerechoSistema coreDerechoQv=new CoreDerechoSistema();
                    if(modo.equals("eliminar")) {
                        coreDerechoQvEJBLocal.remove(coreDerechoQvEJBLocal.find(Integer.parseInt(id)));
                        ...
                        if(!id.equals(""))
                            coreDerechoQv.setCoreDerechoSistemaId(Integer.parseInt(id));
                        else
                            coreDerechoQv.setCoreDerechoSistemaId(null);
                        coreDerechoQv.setDescripcion(request.getParameter("descripcion").toUpperCase());
                        coreDerechoQv.setDerecho(request.getParameter("derecho").toUpperCase());
                        response.getWriter().print(coreDerechoQvEJBLocal.persistir(coreDerechoQv,modo));                          
                    }
                } 
    ...
    private String fieldParse(String campo){
        switch (campo) {
            case "coreDerechoSistemaId":
                return "CORE_DERECHO_SISTEMA_ID";
            default:
                return campo;
        }
    }
