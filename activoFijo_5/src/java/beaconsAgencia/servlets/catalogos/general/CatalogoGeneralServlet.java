...
import beaconsAgencia.ejb.CoreCatalogoGeneralFacadeLocal;
 ...
 * @author jgonzalezc
 */
@WebServlet(name = "CatalogoGeneralServlet", urlPatterns = {"/catalogos/generals/catalogoGeneral.do"})
public class CatalogoGeneralServlet extends HttpServlet {

        @EJB
        CoreCatalogoGeneralFacadeLocal catalogoGeneralEJBLocal;
    /**
            ...
            try {
                ...
                if ( !isMultipart )
                {
                    if(request.getParameter("modo")==null) {
                        int rows=10;
                        int page=1;   
                        String sort="CORE_CATALOGO_GENERAL_ID"; 
                        String order="desc"; 
                        String filter="";
                        ...
                        response.getWriter().print(catalogoGeneralEJBLocal.select(rows,page,sort,order,filter)); 
                    }else {
                        String modo=request.getParameter("modo");
                        String id=request.getParameter("coreCatalogoGeneralId");
                        CoreCatalogoGeneral catalogo=new CoreCatalogoGeneral();
                        if(modo.equals("eliminar")) {
                            catalogoGeneralEJBLocal.remove(catalogoGeneralEJBLocal.find(Integer.parseInt(id)));
                            ...
                            if(!id.equals(""))
                                catalogo=catalogoGeneralEJBLocal.find(Integer.parseInt(id));
                            else
                                catalogo.setCoreCatalogoGeneralId(null);
                            catalogo.setDominio(request.getParameter("dominio").toUpperCase());
                            catalogo.setValor(request.getParameter("valor").replaceAll("\"","'"));
                            catalogo.setAtributo1(request.getParameter("atributo1"));
                            catalogo.setAtributo2(request.getParameter("atributo2"));
                            catalogo.setEsBorrable(request.getParameter("esBorrable"));
                            catalogo.setEstatus(request.getParameter("estatus"));
                            response.getWriter().print(catalogoGeneralEJBLocal.persistir(catalogo,modo));                          
                        }else if(modo.equals("combo")) {
                            response.getWriter().print(catalogoGeneralEJBLocal.generaCombo(request.getParameter("filtro"),"DOS"));
                        }else if(modo.equals("combo1")) {
                            response.getWriter().print(catalogoGeneralEJBLocal.generaCombo1(request.getParameter("filtro"),"DOS"));
                        }else if(modo.equals("comboS")) {
                            response.getWriter().print(catalogoGeneralEJBLocal.generaComboS(request.getParameter("filtro"),"DOS"));
                        }
                    }                 
                }else {   
                    ...
                    while ( iter.hasNext() )
                    {
                        ...
                        {
                            response.getWriter().print(catalogoGeneralEJBLocal.cargarCsv(item.openStream()));
                            break;
                        }
                    }
                }
                ...
                if(request.getParameter("modo")==null)
                    response.getWriter().print("{\"data\":[],\"total\":0,\"filter\":\"\"}"); 
                else
    ...
    private String fieldParse(String campo){
            switch (campo) {
                case "coreCatalogoGeneralId":
                    return "CATALOGO_GENERAL_ID";
                case "esBorrable":
                    return "ES_BORRABLE";
                default:
                    return campo;
